...
char token[MAXN], line[LENGTH], path[100];
int num = 0;
int cc = 0, ll = 0, l = 0, cnt = 0;
FILE *fin, *fout;
const char* reserved_word[] = {"int", "char", "const", "void", "main", "switch", "case",
                                "default", "if", "while", "scanf", "printf", "return"};
enum symbol {PLUSSY, MINUSSY, MULTSY, DIVSY, LSSY, LESY, GTSY, GESY, NEQSY, EQUSY, ASSIGNSY,
             LPARSY, RPARSY, LBRACKSY, RBRACKSY, LPRTSY, RPRTSY, COLONSY, COMMASY, SEMISY, SQUOSY,
             DQUOSY, INTSY, CHARSY, CONSTSY, VOIDSY, MAINSY, SWITCHSY, CASESY, DEFAULTSY,
             IFSY, LOOPSY, SCANFSY, PRINTSY, RETSY, INTEGERSY, CHARACTERSY, STRINGSY, IDENTSY};
symbol result;
...
char getchr(){
    char ch;
    if (cc == ll) { // means we need to read next line.
        cnt ++;
        if (feof(fin)){
...
//    else if (ch == '='){ token[l ++] = ch; result = EQUSY;}
